...
首次执行 `./scripts/macos-bridge.sh deploy` / `setup` 时，脚本会先在终端里提示你填写或确认关键配置，其中 Discord Bot Token 会以 `CODEX_TUNNING_DISCORD_BOT_TOKEN` 的名字单独保存到 `/Users/<user>/.codex-tunning/secrets.env`，不会写进项目 `.env`。
