...
export function formatHelp(prefix: string): string {
  return [
    ...
    `- 当前绑定引擎的全局模型：\`${prefix}model status\` / \`${prefix}model set gpt-5.5\``,
    `- 当前绑定引擎的项目模型：\`${prefix}model project status\` / \`${prefix}model project set gpt-5.5\` / \`${prefix}model project clear\``,
    ...
    `- Codex 全局推理强度：\`${prefix}effort status\` / \`${prefix}effort set <minimal|low|medium|high|xhigh|max|ultra>\` / \`${prefix}effort clear\``,
    `- Codex 项目推理强度：\`${prefix}effort project status\` / \`${prefix}effort project set <minimal|low|medium|high|xhigh|max|ultra>\` / \`${prefix}effort project clear\``,
