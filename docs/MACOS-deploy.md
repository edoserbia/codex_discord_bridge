...
- `#proj-api` 里的线程 `修登录` → `/Users/mac/work/api` 下的一条独立 Codex 会话
- `#proj-api` 里的线程 `重构权限` → `/Users/mac/work/api` 下的另一条独立 Codex 会话
...
```env
ALLOWED_WORKSPACE_ROOTS=/Users/mac/work,/Users/mac/projects,/Users/mac/Desktop/projects
```
...
```text
/Users/mac/work,/Users/mac/projects
```
...
如果你的 `/Users/mac/.openclaw/openclaw.json` 里已经有代理配置，脚本会先自动带入。
...
- 你把 `#proj-api` 绑定到了 `/Users/mac/work/api`
- 那么这个频道和它下面线程里的所有会话，都会在 `/Users/mac/work/api` 里运行
...
它会直接进入这个项目目录，启动/续用这个频道自己的 Codex 会话。实时进度消息里会展示分析摘要、计划清单、命令执行和输出预览。
