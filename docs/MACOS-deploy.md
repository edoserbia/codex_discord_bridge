...
- 当前绑定目录：`<workspace>/.claude/settings.json`
- 全局 Claude settings：默认 `~/.claude/settings.json`，可用 `.env` 的 `CLAUDE_SETTINGS_PATH=...` 覆盖
- Claude CLI 自己的默认值
...
Bridge 会尽量用非交互方式调用 Claude CLI。如果 Claude 仍然请求工具权限，Discord 中会出现类似提示：
...
管理员发送 `!approve <请求ID>` 后，Bridge 只会把该规则写入当前项目目录的 `.claude/settings.json`。如果原请求没有附件，Bridge 会自动重试原任务；如果原请求包含附件，请重新发送，避免引用已清理的临时文件。
